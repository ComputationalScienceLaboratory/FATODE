...
! Observation mapping operator
subroutine HMap(U, obs)

  use swe2Dxy_parameters
  implicit none

  double precision, intent(in)  ::  U(3,Mx+4,My+4)
  double precision, intent(out) ::  obs(obsSpaceSize)

  integer :: k, i, j
  integer :: indx


  if (obsSpaceSize .EQ. ndim) then
...
! Observation inverse mapping operator
subroutine HInverseMap(obs,U)

  use swe2Dxy_parameters
  implicit none

  double precision, intent(out) ::  U(3,Mx+4,My+4)
  double precision, intent(in)  ::  obs(obsSpaceSize)
...
do p = 1, Mx*My
  ...
  if (feigvalues(2,i,j) .GE. 0d0) then
    ...
    if ((ind .GE. 1) .AND. (ind .LE. ndim)) then
      ...
      & (0.0416667*g*(3*C2 - 6*N2 + NN2 + 2*S2))/(dx*(C1*g)**1.5)) &
      & + (-(C2/C1**2) - g/(2.*SqrtC1g))*(((0.5 + (0.5*C2)/(C1*SqrtC1g)) &
      & *(3*C1 - 6*N1 + NN1 + 2*S1))/(6.*dx) - (0.0833333*(3*C2 - 6*N2 + NN2 &
      & + 2*S2))/(dx*SqrtC1g))
    end if  
    ...
    if ((ind .GE. 1) .AND. (ind .LE. ndim)) then
      JF(p,ind)=JF(p,ind)+(C2/C1 - SqrtC1g)*(-0.25/(dx*SqrtC1g) + (0.0833333*(3*C1 &
      & - 6*N1 + NN1 + 2*S1))/(C1*dx*SqrtC1g)) + (((0.5 + (0.5*C2)/(C1*SqrtC1g))&
      & *(3*C1 - 6*N1 + NN1 + 2*S1))/(6.*dx) - (0.0833333*(3*C2 - 6*N2 + NN2 + 2*S2))&
      & /(dx*SqrtC1g))/C1
    end if  
  ...
  if (feigvalues(3,i,j) .GE. 0d0) then
    ...
    if ((ind .GE. 1) .AND. (ind .LE. ndim)) then
      ...
      & + (0.0833333*(3*C2 - 6*N2 + NN2 + 2*S2))/(dx*SqrtC1g))
    end if  
    call mapgr(ind,p,0,1,0)  !S1
    if ((ind .GE. 1) .AND. (ind .LE. ndim)) then
      JF(p,ind)=JF(p,ind)+((0.5 - (0.5*C2)/(C1*SqrtC1g))*(C2/C1 + SqrtC1g))/(3.*dx)
    ...
    call mapgr(ind,p,1,0,0)  !C2
    if ((ind .GE. 1) .AND. (ind .LE. ndim)) then
      JF(p,ind)=JF(p,ind)+(C2/C1 + SqrtC1g)*(0.25/(dx*SqrtC1g) - (0.0833333*(3*C1 - 6*N1 + NN1 + 2*S1))&
      & /(C1*dx*SqrtC1g)) + (((0.5 - (0.5*C2)/(C1*SqrtC1g))*(3*C1 - 6*N1 + NN1 + 2*S1))&
      & /(6.*dx) + (0.0833333*(3*C2 - 6*N2 + NN2 + 2*S2))/(dx*SqrtC1g))/C1
    end if  
    call mapgr(ind,p,1,1,0)  !S2
    if ((ind .GE. 1) .AND. (ind .LE. ndim)) then
      JF(p,ind)=JF(p,ind)+(0.166667*(C2/C1 + SqrtC1g))/(dx*SqrtC1g)
...
!776	continue
  ...
  if (geigvalues(2,i,j) .GE. 0d0) then
    ...
    if ((ind .GE. 1) .AND. (ind .LE. ndim)) then
      ...
      & (0.5*C3)/(C1*SqrtC1g))*(3*C1 + 2*E1 - 6*W1 + WW1))/(6.*dy) &
  ...
  if (geigvalues(3,i,j) .GE. 0d0) then
    ...
    call mapgr(ind,p,2,0,0)  !C3
    if ((ind .GE. 1) .AND. (ind .LE. ndim)) then
      JG(p,ind)=JG(p,ind)+(C3/C1 + SqrtC1g)*(-0.25/(dy*SqrtC1g) &
      & - (0.0833333*(-3*C1 + 6*E1 - EE1 - 2*W1))/(C1*dy*SqrtC1g)) +&
      & (((0.5 - (0.5*C3)/(C1*SqrtC1g))*(-3*C1 + 6*E1 - EE1 - 2*W1))/(6.*dy)&
      & +(0.0833333*(-3*C3 + 6*E3 - EE3 - 2*W3))/(dy*SqrtC1g))/C1
    end if  
    call mapgr(ind,p,2,0,1)  !E3
...
!goto 787
  ...
  if (geigvalues(2,i,j) .GE. 0d0) then
    ...
    call mapgr(ind,p,-1,0,0)  !C1
    if ((ind .GE. 1) .AND. (ind .LE. ndim)) then
      JG(p,ind)=JG(p,ind)+(C3/C1 - SqrtC1g)*((0.25*C2*(1 + C3/(C1*SqrtC1g)))&
      & /(C1*dy) +&
      & (0.0833333*C2*(-(C3*g)/(2.*C1*(C1*g)**1.5) &
      & - C3/(C1**2*SqrtC1g))*(3*C1 + 2*E1 - 6*W1 + WW1))/(C1*dy) -&
      & (0.0833333*C2*(1 + C3/(C1*SqrtC1g))*(3*C1 + 2*E1 - 6*W1 + WW1))&
      & /(C1**2*dy)+(0.0416667*C2*g*(3*C3+2*E3-6*W3+WW3))/(C1*dy*(C1*g)**1.5)+ &
      & (0.0833333*C2*(3*C3 + 2*E3 - 6*W3 + WW3))/(C1**2*dy*SqrtC1g)) &
      & + (-(C3/C1**2) - g/(2.*SqrtC1g))*((0.0833333*C2*(1 + C3/(C1*SqrtC1g))&
      & *(3*C1 + 2*E1 - 6*W1 + WW1))/(C1*dy) - (0.0833333*C2*(3*C3 + 2*E3 &
      & - 6*W3 + WW3))/(C1*dy*SqrtC1g))
    end if  
    call mapgr(ind,p,-1,0,1)  !E1
    if ((ind .GE. 1) .AND. (ind .LE. ndim)) then
	...
	call mapgr(ind,p,0,0,0)   !C2
    if ((ind .GE. 1) .AND. (ind .LE. ndim)) then
      JG(p,ind)=JG(p,ind)+(C3/C1 - SqrtC1g)*((0.0833333*(1 + C3/(C1*SqrtC1g))&
      & *(3*C1 + 2*E1 - 6*W1 + WW1))/(C1*dy) - (0.0833333*(3*C3 + 2*E3 &
      & - 6*W3 + WW3))/(C1*dy*SqrtC1g))
    end if  
    ...
    if ((ind .GE. 1) .AND. (ind .LE. ndim)) then
      ...
      & ((0.0833333*C2*(1 + C3/(C1*SqrtC1g))*(3*C1 + 2*E1 - 6*W1 + WW1))&
      & /(C1*dy) - (0.0833333*C2*(3*C3 + 2*E3 - 6*W3 + WW3))/(C1*dy*SqrtC1g))/C1
    end if  
    ...
    if ((ind .GE. 1) .AND. (ind .LE. ndim)) then
      JG(p,ind)=JG(p,ind)+(C3/C1 - SqrtC1g)*((-0.25*C2*(1 + C3/(C1*SqrtC1g)))&
      & /(C1*dy) + (0.0833333*C2*(-(C3*g)/(2.*C1*(C1*g)**1.5) -&
  ...
  if (geigvalues(3,i,j) .GE. 0d0) then
    ...
    if ((ind .GE. 1) .AND. (ind .LE. ndim)) then
      ...
      & + (0.0833333*C2*((C3*g)/(2.*C1*(C1*g)**1.5) +&
      & C3/(C1**2*SqrtC1g))*(3*C1 + 2*E1 - 6*W1 + WW1))/(C1*dy)-(0.0833333*C2&
      & *(1 - C3/(C1*SqrtC1g))*(3*C1 + 2*E1 - 6*W1 + WW1))/(C1**2*dy) -&
      & (0.0416667*C2*g*(3*C3 + 2*E3 - 6*W3 + WW3))/(C1*dy*(C1*g)**1.5) &
      & - (0.0833333*C2*(3*C3 + 2*E3 - 6*W3 + WW3))/(C1**2*dy*SqrtC1g)) &
      & + (-(C3/C1**2) +&
      & g/(2.*SqrtC1g))*((0.0833333*C2*(1-C3/(C1*SqrtC1g))*(3*C1 + 2*E1-6*W1&
      &  + WW1))/(C1*dy) +&
    ...
    if ((ind .GE. 1) .AND. (ind .LE. ndim)) then
      ...
      & - (0.0833333*C2*(-3*C3 + 6*E3 - EE3 - 2*W3))/(C1**2*dy*SqrtC1g)) &
      & + (-(C3/C1**2) +&
      & g/(2.*SqrtC1g))*((0.0833333*C2*(1 - C3/(C1*SqrtC1g))*(-3*C1 + 6*E1 &
      & - EE1 - 2*W1))/(C1*dy) + (0.0833333*C2*(-3*C3 + 6*E3 - EE3 - 2*W3))&
      & /(C1*dy*SqrtC1g))
    end if  
    call mapgr(ind,p,-1,0,1)  !E1
    if ((ind .GE. 1) .AND. (ind .LE. ndim)) then
    ...
    if ((ind .GE. 1) .AND. (ind .LE. ndim)) then
      JG(p,ind)=JG(p,ind)+(C3/C1 + SqrtC1g)*((0.0833333*(1 - C3/(C1*SqrtC1g))&
      & *(-3*C1 + 6*E1 - EE1 - 2*W1))/(C1*dy) + (0.0833333*(-3*C3 + 6*E3 - &
...
!goto 796
  ...
  if (feigvalues(1,i,j) .GE. 0d0) then
    ...
    if ((ind .GE. 1) .AND. (ind .LE. ndim)) then
      JF(p,ind)=JF(p,ind)+(C2*(-C3/(2.*C1*dx) + (C3*(3*C1 - 6*N1 + NN1 + 2*S1))&
      & /(6.*C1**2*dx)))/C1 - (C2*(-(C3*(3*C1 - 6*N1 + NN1 + 2*S1))/(6.*C1*dx)&
      &  + (3*C3 - 6*N3 + NN3 + 2*S3)/(6.*dx)))/C1**2
    end if  
    ...
    call mapgr(ind,p,-2,0,0)  !C1
    if ((ind .GE. 1) .AND. (ind .LE. ndim)) then
      JF(p,ind)=JF(p,ind)+(C2*(C3/(2.*C1*dx) + (C3*(-3*C1 - 2*N1 + 6*S1 - SS1))&
      & /(6.*C1**2*dx)))/C1 - (C2*(-(C3*(-3*C1 - 2*N1 + 6*S1 - SS1))&
      & /(6.*C1*dx) + (-3*C3 - 2*N3 + 6*S3 - SS3)/(6.*dx)))/C1**2
    end if  
    call mapgr(ind,p,-2,1,0)  !S1
    if ((ind .GE. 1) .AND. (ind .LE. ndim)) then
      JF(p,ind)=JF(p,ind)-((C2*C3)/(C1**2*dx))
  ...
  if (feigvalues(2,i,j) .GE. 0d0) then
    ...
    call mapgr(ind,p,0,0,0)   !C3
    if ((ind .GE. 1) .AND. (ind .LE. ndim)) then
      JF(p,ind)=JF(p,ind)+(C2/C1 - SqrtC1g)*((0.0833333*(1 + C2/(C1*SqrtC1g))&
      & *(3*C1 - 6*N1 + NN1 + 2*S1))/(C1*dx) - (0.0833333*(3*C2 - 6*N2 + NN2 &
      & + 2*S2))/(C1*dx*SqrtC1g))
    end if  
    ...
    if ((ind .GE. 1) .AND. (ind .LE. ndim)) then
      ...
      & ((0.0833333*C3*(1 + C2/(C1*SqrtC1g))*(-3*C1 - 2*N1 + 6*S1 - SS1))&
      & /(C1*dx) - (0.0833333*C3*(-3*C2 - 2*N2 + 6*S2 - SS2))/(C1*dx*SqrtC1g))/C1
    end if  
    call mapgr(ind,p,-1,1,0)  !S2
    if ((ind .GE. 1) .AND. (ind .LE. ndim)) then
      JF(p,ind)=JF(p,ind)+(-0.5*C3*(C2/C1 - SqrtC1g))/(C1*dx*SqrtC1g)
  ...
  if (feigvalues(3,i,j) .GE. 0d0) then
    ...
    call mapgr(ind,p,-2,0,0)  !C1
    if ((ind .GE. 1) .AND. (ind .LE. ndim)) then
      JF(p,ind)=JF(p,ind)+(C2/C1 + SqrtC1g)*((0.25*C3*(1 - C2/(C1*SqrtC1g)))&
      & /(C1*dx) +&
      & (0.0833333*C3*((C2*g)/(2.*C1*(C1*g)**1.5) + C2/(C1**2*SqrtC1g))&
      & *(3*C1 - 6*N1 + NN1 + 2*S1))/(C1*dx) -&
      & (0.0833333*C3*(1 - C2/(C1*SqrtC1g))*(3*C1 - 6*N1 + NN1 + 2*S1))&
      & /(C1**2*dx)-(0.0416667*C3*g*(3*C2-6*N2+NN2+2*S2))/(C1*dx*(C1*g)**1.5)-&
      & (0.0833333*C3*(3*C2 - 6*N2 + NN2 + 2*S2))/(C1**2*dx*SqrtC1g)) &
      & + (-(C2/C1**2) + g/(2.*SqrtC1g))*((0.0833333*C3*(1 - C2/(C1*SqrtC1g))&
      & *(3*C1 - 6*N1 + NN1 + 2*S1))/(C1*dx) + (0.0833333*C3*(3*C2 - 6*N2 & 
      & + NN2 + 2*S2))/(C1*dx*SqrtC1g))
    end if  
    call mapgr(ind,p,-2,1,0)  !S1
    if ((ind .GE. 1) .AND. (ind .LE. ndim)) then
      JF(p,ind)=JF(p,ind)+(0.166667*C3*(1 - C2/(C1*SqrtC1g))*(C2/C1 + SqrtC1g))/(C1*dx)
    ...
    if ((ind .GE. 1) .AND. (ind .LE. ndim)) then
      ...
      & (0.0833333*C3*((C2*g)/(2.*C1*(C1*g)**1.5)+C2/(C1**2*SqrtC1g))*(-3*C1-2*N1 + 6*S1 - SS1))/(C1*dx) -&
      & (0.0833333*C3*(1 - C2/(C1*SqrtC1g))*(-3*C1 - 2*N1 + 6*S1 - SS1))&
      & /(C1**2*dx)-(0.0416667*C3*g*(-3*C2-2*N2+6*S2-SS2))/(C1*dx*(C1*g)**1.5)-&
      & (0.0833333*C3*(-3*C2 - 2*N2 + 6*S2 - SS2))/(C1**2*dx*SqrtC1g)) &
      & + (-(C2/C1**2) + g/(2.*SqrtC1g))*((0.0833333*C3*(1-C2/(C1*SqrtC1g))&
      & *(-3*C1-2*N1+6*S1-SS1))/(C1*dx)+(0.0833333*C3*(-3*C2-2*N2+6*S2-SS2))&
      & /(C1*dx*SqrtC1g))
    end if  
    call mapgr(ind,p,-2,1,0)  !S1
    if ((ind .GE. 1) .AND. (ind .LE. ndim)) then
...
!goto 797
  ...
  if (geigvalues(3,i,j) .GE. 0d0) then
    ...
    if ((ind .GE. 1) .AND. (ind .LE. ndim)) then
      JG(p,ind)=JG(p,ind)+(C3/C1 + SqrtC1g)*((0.25*(-(C3**2/C1**2) + C1*g))/(dy*SqrtC1g) +&
      & (0.0833333*((2*C3**2)/C1**3 + g)*(3*C1 + 2*E1 - 6*W1 + WW1))/(dy*SqrtC1g) -&
      & (0.0416667*g*(-(C3**2/C1**2) + C1*g)*(3*C1 + 2*E1 - 6*W1 + WW1))/(dy*(C1*g)**1.5) &
    ...
    if ((ind .GE. 1) .AND. (ind .LE. ndim)) then
      ...
      & g/(2.*SqrtC1g))*((0.0833333*(-(C3**2/C1**2) + C1*g)*(-3*C1 + 6*E1 - EE1 - 2*W1))&
      & /(dy*SqrtC1g) + ((0.5 + (0.5*C3)/(C1*SqrtC1g))*(-3*C3 + 6*E3 - EE3 - 2*W3))/(6.*dy))
    end if  
    call mapgr(ind,p,-2,0,1)  !E1
    if ((ind .GE. 1) .AND. (ind .LE. ndim)) then
      JG(p,ind)=JG(p,ind)+(0.5*(-(C3**2/C1**2) + C1*g)*(C3/C1 + SqrtC1g))/(dy*SqrtC1g)
