...
module swe2Dxy_Parameters
  ...
  integer, parameter ::  nvar = 3*cells
  ...
  !integer, parameter   :: obsSpaceSize = nvar
  ...
  double precision                      :: Zeta(30, nvar)
  !~~> Current initial state and background state
  double precision                      :: back(nvar), ref(nvar), ic(nvar)
...
!goto 796
  ...
  if (feigvalues(1,i,j) .GE. 0d0) then
    ...
    if ((ind .GE. 0) .AND. (ind .LE. nvar)) then
