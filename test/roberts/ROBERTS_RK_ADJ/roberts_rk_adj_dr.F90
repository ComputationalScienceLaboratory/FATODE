...
program roberts_ros_adj_dr
  ...
  implicit none
  external fun,jac,DRDP,DRDY,JACP,AdjInit,QFUN
  ...
  integer ::istate(20),icntrl(20)
    ...
    print *, '     G = R = int_t0^tB0 r(t,p,y) dt '
    print *, '     r(t,p,y) = y(t,p) '
...
! change initial step size
    ...
    call integrate_adj(tin=tstart, tout=tend, nvar=nvar, np=np, nnzero=nnz,&
        ...
        drdp =DRDP, drdy=DRDY, jacp=JACP,adjinit=AdjInit,&
        rstatus_u=rstate,rcntrl_u=rcntrl, istatus_u=istate,&
icntrl_u=icntrl,q=q, qfun=qfun)
    call cpu_time(t2)
    tcpu=t2-t1
