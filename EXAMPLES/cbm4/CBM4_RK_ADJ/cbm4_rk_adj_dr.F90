...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  double precision, intent(in) ::t,y(nvar)
  double precision, intent(inout) :: fpjac(nvar,np)
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  integer, intent(in) :: n
  double precision, intent(in) ::t,y(nvar)
  double precision, intent(inout) :: fjac(nvar,nvar)
  fjac(:,:) = 0.0d0
  call update_sun(t)
  call update_rconst()
...
!~~~> initialize adjoint variables
  ...
  open(unit=12,file='cbm4_output_sens.txt',status='replace')

  do i=1,nadj
    write(12,'(A,I3)') 'Lambda: column',i
    do j=1,nvar
      write(12,'(E24.16)') lambda(j,i)
    end do
  end do

  do i=1,nadj
    write(12,'(A,I3)') 'Mu: column',i
    do j=1,np
      write(12,'(E24.16)') mu(j,i) 
!      write(12,'("$",ES12.2,"$")') mu(j,i)
    end do
  end do
  close(12)  
