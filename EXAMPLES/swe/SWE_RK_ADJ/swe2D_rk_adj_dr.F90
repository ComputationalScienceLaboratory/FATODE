...
!      close(11)
    ...
    subroutine output_sen(ndim,nadj,y_adj,fno)
      ...
      write(fname,'(a,i3.3,a)') 'RK_ADJ_',fno,'.m'
  ...
  program swe_rk_adj_dr
    ...
    allocate(U_par(3,mx+4,my+4),temp_par(3, mx+4, my+4), feigvalues_par(3, mx+4, my+4), &
                geigvalues_par(3, mx+4,my+4), stat=state)
    if (state .ne. 0) then
...
!       rcntrl(3)=1e-3
     ...
     deallocate(U_par,temp_par, feigvalues_par, geigvalues_par, stat = state)
     if (state .ne. 0) then
       print *, 'deallocation error of u,feigvalues,geigvalues'
       stop 1
     end if
