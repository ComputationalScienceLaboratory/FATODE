...
!      close(11)
    ...
    subroutine output_sen(n,ntlm,y_tlm,fno)
      ...
      write(fname,'(a,i3.3,a)') 'ERK_TLM_',fno,'.m'
  ...
  program swe_rk_tlm_dr
    ...
    if (state .ne. 0) then
     ...
     do j = 1,6
       ...
       istate(:)=0
       icntrl(:)=0
       icntrl(3) = 4  ! ~~~> choose method
       rcntrl(:)=0
! change initial step size
!       rcntrl(3)=1e-3
      
       call cpu_time(t1)
       call integrate_tlm(tin=tstart, tout=tend, nvar=nvar, &
        var_tlm=y_tlm, var=var, rtol=rtol, atol=atol, ntlm=ntlm,&
        fun=fun, jac=jac, rstatus_u=rstate,rcntrl_u=rcntrl, &
