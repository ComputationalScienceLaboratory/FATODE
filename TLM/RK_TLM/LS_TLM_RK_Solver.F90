...
!      integer, allocatable ::  ip_big(:)
      double precision, allocatable :: e1(:,:)
!     double precision, allocatable :: e_big(:,:)
...
contains
    subroutine lapack_init(n)
      integer :: n,state
      nvar = n
      allocate(ip1(nvar),ip2(nvar),fjac(nvar,nvar),fjac1(nvar,nvar),&
        fjac2(nvar,nvar),fjac3(nvar,nvar),e1(nvar,nvar),e2(nvar,nvar),STAT=state)
      if(state .ne. 0) then
        stop 'ALlocation error in lapack_init'
      end if
!      allocate(ip_big(3*nvar), e_big(3*nvar,3*nvar),STAT=state)
!      if(state .ne. 0) then
!        stop 'ALlocation error in lapack_init'
!      end if
    ...
    subroutine lapack_free
      integer :: state
      deallocate(ip1,ip2,fjac,fjac1,fjac2,fjac3,e1,e2,STAT=state)
      if(state .ne. 0) then
        stop 'DeaLlocation error in lapack_free'
      end if

!      deallocate(ip_big,e_big,STAT=state)
!      if(state .ne. 0) then
!        stop 'DeaLlocation error in lapack_free'
!      end if
...
!ax(nnz),axc(nnz),azc(nnz),ai(nnz),ap(nvar+1),ax_big(9*nnz),ai_big(9*nnz),ap_big(3*nvar+1)
    ...
    double precision, allocatable :: ax_big(:)
    integer, allocatable :: ai_big(:),ap_big(:)
...
!     free previously used memory
    ...
    call umf4num(ap_big, ai_big, ax_big, symbolic_big, numeric_big, control,&
                            info)
    if (info(1) .lt. 0) then                                
      print *, 'error occurred in umf4num: ', info(1)       
      stop
...
!~~~>Clean objects created during the process
      ...
      deallocate(ax,axc,azc,ai,ap,ax_big,ai_big,ap_big,fjac,fjac1,fjac2,fjac3,STAT=state)
      if(state .ne. 0) then
        stop 'Deallocation error in umf_free'
      end if
      end subroutine umf_free

end module umf_tlm_rk
#endif

#ifdef SPARSE_LU
!~~~>SuperLU implementation
module superlu_tlm_rk
    implicit none
    save
    integer :: nvar, nnz
! ax(nnz),b(nvar),ai(nnz),ap(nvar+1),axc(nnz),bc(nvar),ax_big(9*nnz),b_big(3*nvar),ai_big(9*nnz),ap_big(3*nvar+1)
    ...
    double precision, allocatable :: ax_big(:),b_big(:)
    integer, allocatable :: ai_big(:),ap_big(:)
...
!~~~>Decomposition of the left hand side (HGamma-Jac)
    subroutine superlu_decomp(hgamma,ising)
      ...
      double precision :: hgamma 
      ! free previously used memory before new LU factorization
      tr = 0
      iopt = 3
      ldb = nvar
      nrhs = 1
...
!            ax(idx) = e(i,j)
  ...
  !   factorize the matrix. The factors are stored in *factors* handle.
      ...
      if(ising .ne. 0) then
        write(*,*) 'INFO from failed factorization in cmp ', ising
      endif
...
!   solve the system using the existing factors.
    ...
    if(info .ne. 0) then
      ...
      factors_cmp = 0
      factors_big = 0 
      allocate(ax(nnz),b(nvar),ai(nnz),ap(nvar+1),axc(nnz),bc(nvar),ax_big(9*nnz),&
                     b_big(3*nvar),ai_big(9*nnz),ap_big(3*nvar+1),fjac(nvar,nvar),&
                    fjac1(nvar,nvar),fjac2(nvar,nvar),fjac3(nvar,nvar), STAT=state)
      if(state .ne. 0) then
      ...
      deallocate(ax,b,ai,ap,axc,bc,ax_big,b_big,ai_big,ap_big,fjac,&
                                      fjac1,fjac2,fjac3,STAT=state)
      if(state .ne. 0) then
        stop 'Deallocation error in superlu_free'
      end if
      end subroutine superlu_free

end module superlu_tlm_rk
#endif

module ls_tlm_rk_solver
#ifdef SPARSE_UMF
      use umf_tlm_rk
#endif
#ifdef FULL_ALGEBRA
...
!      print *, rhs                                           
     ...
     subroutine lss_solve_big(trans,zbig)
      implicit none
      double precision :: zbig(3*nvar)
      logical :: trans
#ifdef SPARSE_UMF  
      call umf_solve_big(trans, zbig)
#endif
#ifdef FULL_ALGEBRA
      call lapack_solve_big(trans, zbig)                     
#endif                                                 
#ifdef SPARSE_LU                                       
      call superlu_solve_big(trans, zbig)                    
#endif                                                 
!      print *, rhs                                         
...
!~~~> multiplication of the jacobian matrix and a vector
     subroutine lss_mul_jac3(z,g)
      implicit none
      double precision :: z(nvar), g(nvar)
      z = matmul(fjac3,g)
     end subroutine lss_mul_jac3
